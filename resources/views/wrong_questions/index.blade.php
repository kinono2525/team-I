                ...
                <div class="p-6 text-gray-900 dark:text-gray-900">
                    <form method="POST" action="{{ route('wrong_questions.store', ['student' => $student->id]) }}" x-data="{ showAll: false }">
                        ...
                        <div class="mt-4 text-right">
                            ...
                            <a href="{{ route('wrong_questions.pdf', ['student' => $student->id]) }}" onclick="event.preventDefault();if(confirm('本当に出力しますか？')) { window.location.href = this.href; setTimeout(() => location.reload(), 10000); }" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">テスト出力</a>                        
                        </div>
                    </form>
                </div>
            </div>
